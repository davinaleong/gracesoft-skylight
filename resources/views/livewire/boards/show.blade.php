...
new class extends Component
{
    ...
    public string $editCardStartsAt = '';
...
<div>
                ...
                >
                    @foreach ($column->cards as $card)
                        ...
                        >
                            @if ($editingCard === $card->id)
                                <form wire:submit="saveCard" class="space-y-2" data-no-drag>
                                    ...
                                    <textarea
                                        ...
                                        rows="3"
                                        placeholder="Description&hellip; (Markdown supported)"
                                        class="w-full rounded border border-gray-300 dark:border-gray-700 bg-transparent px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-y min-h-16"
                                    ></textarea>
                                    <div class="grid grid-cols-2 gap-1.5">
                                        <div>
                                            <label class="block text-xs text-gray-500 dark:text-gray-400 mb-0.5">Start date</label>
                                            <input type="date" wire:model="editCardStartsAt"
                                                class="w-full rounded border border-gray-300 dark:border-gray-700 bg-transparent px-2 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500">
                                            @error('editCardStartsAt') <p class="text-xs text-red-600">{{ $message }}</p> @enderror
                                        </div>
                                        <div>
                                            <label class="block text-xs text-gray-500 dark:text-gray-400 mb-0.5">Due date</label>
                                            <input type="date" wire:model="editCardEndsAt"
                                                class="w-full rounded border border-gray-300 dark:border-gray-700 bg-transparent px-2 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500">
                                            @error('editCardEndsAt') <p class="text-xs text-red-600">{{ $message }}</p> @enderror
                                        </div>
                                    </div>
                            ...
                            @else
                                <div @click="$wire.set('openCardId', {{ $card->id }})" class="cursor-pointer">
                                    <div class="flex items-start justify-between gap-1.5">
                                        <div class="flex items-start gap-1.5 flex-1 min-w-0">
                                            <div data-drag-handle @click.stop="" class="mt-0.5 shrink-0 cursor-grab active:cursor-grabbing text-gray-300 dark:text-gray-600 hover:text-gray-500 dark:hover:text-gray-400 opacity-0 group-hover/card:opacity-100 transition-opacity" aria-label="Drag card">
                                            ...
                                            <span class="text-sm leading-snug flex-1">{{ $card->title }}</span>
                                        </div>
                                        <div class="flex shrink-0 gap-0.5 opacity-0 group-hover/card:opacity-100 transition-opacity">
                                            <button @click.stop="$wire.startEditCard({{ $card->id }})" class="rounded p-1 text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700" aria-label="Edit card">
                                                <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125" /></svg>
                                            </button>
                                            <button @click.stop="$wire.deleteCard({{ $card->id }})" wire:confirm="Delete this card?" class="rounded p-1 text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20" aria-label="Delete card">
                                    ...
                                    @if ($boardLabels->isNotEmpty())
                                        <div class="mt-2 opacity-0 group-hover/card:opacity-100 transition-opacity">
                                            <div class="flex flex-wrap gap-1">
                                                @foreach ($boardLabels as $label)
                                                    <button
                                                        @click.stop="$wire.toggleCardLabel({{ $card->id }}, {{ $label->id }})"
                                    ...
                                    @endif
                                    @if ($card->description)
                                        <div class="mt-1.5 card-prose-sm text-gray-500 dark:text-gray-400 line-clamp-3">
    ...
    {{-- Card detail modal --}}
    @if ($openCardId)
        @php $openCard = \App\Models\Card::with('column')->find($openCardId); @endphp
        @if ($openCard)
            <div
            ...
            >
                {{-- Backdrop --}}
                <div wire:click="$set('openCardId', null)" class="absolute inset-0 bg-black/50 backdrop-blur-sm"></div>

                {{-- Modal --}}
                <div class="relative z-10 flex flex-col w-full max-w-2xl max-h-[90vh] rounded-2xl bg-white dark:bg-gray-900 shadow-2xl">
                    {{-- Modal header --}}
                    <div class="flex items-center justify-between shrink-0 border-b border-gray-100 dark:border-gray-800 px-6 py-4">
                        <p class="text-xs font-semibold uppercase tracking-widest text-gray-400">{{ $openCard->column->name }}</p>
                        <button wire:click="$set('openCardId', null)" class="rounded-lg p-1.5 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors" aria-label="Close">
                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" /></svg>
                        </button>
                    </div>
